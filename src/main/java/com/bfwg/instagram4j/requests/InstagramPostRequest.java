...
public abstract class InstagramPostRequest<T> extends InstagramRequest<T> {
    ...
    public T execute() throws ClientProtocolException, IOException {
        HttpPost post = new HttpPost(com.bfwg.instagram4j.InstagramConstants.API_URL + getUrl());
        ...
        log.debug("User-Agent: " + com.bfwg.instagram4j.InstagramConstants.USER_AGENT);
        String payload = getPayload();
        log.debug("Base Payload: " + payload);
        
        if (isSigned()) {
            payload = com.bfwg.instagram4j.util.InstagramHashUtil.generateSignature(payload);
        }
